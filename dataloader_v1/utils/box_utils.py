...
def jitter_boxes(boxes, noise_scale=0.025):
    ...
    bbox_jitters = torch.empty(boxes.get_shape()).normal_(mean = 0, std = noise_scale)
    ymin = boxes[..., 0:1]
    xmin = boxes[..., 1:2]
    ymax = boxes[..., 2:3]
    xmax = boxes[..., 3:4]
...
def normalize_boxes(boxes, image_shape):
    ...
    if isinstance(image_shape, list) or isinstance(image_shape, tuple):
      ...
      image_shape = image_shape.type(boxes.dtype)
    ...
    normalized_boxes = torch.cat([ymin, xmin, ymax, xmax],dim = -1)
...
def denormalize_boxes(boxes, image_shape):
    ...
    if isinstance(image_shape, list) or isinstance(image_shape, tuple):
      ...
      height, width = torch.split(image_shape, 2, dim=-1)

    # ymin, xmin, ymax, xmax = tf.split(boxes, 4, axis=-1)
    ymin, xmin, ymax, xmax = torch.split(boxes, 4, dim=-1)
    ymin = ymin * height
    xmin = xmin * width
    ymax = ymax * height
    xmax = xmax * width
    ...
    denormalized_boxes = torch.cat([ymin, xmin, ymax, xmax],dim = -1)
...
def clip_boxes(boxes, image_shape):
    ...
    if isinstance(image_shape, list) or isinstance(image_shape, tuple):
      ...
      height = image_shape[..., 0:1]
      width = image_shape[..., 1:2]
      # y_x_max_list = tf.concat([height - 1.0, width - 1.0, height - 1.0, width - 1.0], -1)
      y_x_max_list = torch.cat([height - 1.0, width - 1.0, height - 1.0, width - 1.0], dim = -1)

    return torch.maximum(torch.minimum(boxes, y_x_max_list), 0.0)
...
def compute_outer_boxes(boxes, image_shape, scale=1.0):
    ...
    outer_boxes = torch.split([centers_y - box_height / 2.0,
                               centers_x - box_width / 2.0,
                               centers_y + box_height / 2.0,
                               centers_x + box_width / 2.0], dim = 1)
...
def encode_boxes(boxes, anchors, weights=None):
    ...
    boxes = boxes.type(anchors.dtype)
    ymin = boxes[..., 0:1]
    xmin = boxes[..., 1:2]
    ymax = boxes[..., 2:3]
    xmax = boxes[..., 3:4]
    ...
    #     axis=-1)
    encoded_boxes = torch.cat([encoded_dy, encoded_dx, encoded_dh, encoded_dw],dim=-1)
...
def decode_boxes(encoded_boxes, anchors, weights=None):
    ...
    dh = torch.minimum(dh, BBOX_XFORM_CLIP)
    # dw = tf.minimum(dw, BBOX_XFORM_CLIP)
    dw = torch.minimum(dw, BBOX_XFORM_CLIP)
    ...
    #       axis=-1)
    decoded_boxes = torch.cat([decoded_boxes_ymin, decoded_boxes_xmin,
                                decoded_boxes_ymax, decoded_boxes_xmax],dim=-1)
...
def filter_boxes(boxes, scores, image_shape, min_size_threshold):
    ...
    min_size = torch.maximum(min_size_threshold, 1.0).type(boxes.dtype)

    # filtered_size_mask = tf.logical_and(
    #       tf.greater(h, min_size), tf.greater(w, min_size))
    filtered_size_mask = torch.logical_and(
            torch.greater(h, min_size), torch.greater(w, min_size))
    ...
    filtered_center_mask = torch.logical_and(
            torch.logical_and(torch.greater(yc, 0.0), torch.less(yc, height)),
            torch.logical_and(torch.greater(xc, 0.0), torch.less(xc, width)))

    # filtered_mask = tf.logical_and(filtered_size_mask, filtered_center_mask)
    filtered_mask = torch.logical_and(filtered_size_mask, filtered_center_mask)
    ...
    filtered_boxes = torch.unsqueeze(filtered_mask, dim=-1).type(boxes.dtype) * boxes
...
def filter_boxes_by_scores(boxes, scores, min_score_threshold):
    ...
    filtered_mask = torch.greater(scores, min_score_threshold)
    #filtered_scores = tf.where(filtered_mask, scores, tf.zeros_like(scores))
    filtered_scores = torch.where(filtered_mask, scores, torch.zeros_like(scores))
    #filtered_boxes = tf.cast(
    #     tf.expand_dims(filtered_mask, axis=-1), dtype=boxes.dtype) * boxes
...
def top_k_boxes(boxes, scores, k):
    ...
    selected_scores, top_k_indices = torch.topk(scores,k,sorted=True)

    batch_size, _ = scores.get_shape().as_list()
    if batch_size == 1:
      # selected_boxes = tf.squeeze(
      #     tf.gather(boxes, top_k_indices, axis=1), axis=1)
      selected_boxes = torch.squeeze(torch.gather(boxes, 1, top_k_indices), dim = 1)

    else:
      # top_k_indices_shape = tf.shape(top_k_indices)
      top_k_indices_shape = top_k_indices.size()
      # batch_indices = (
      #       tf.expand_dims(tf.range(top_k_indices_shape[0]), axis=-1) *
      #       tf.ones([1, top_k_indices_shape[-1]], dtype=tf.int32))
      batch_indices = (
              torch.unsqueeze(torch.range(start = 0, end = top_k_indices_shape[0]), dim = -1) *
              torch.ones([1, top_k_indices_shape[-1]],dtype=torch.int32))
      # gather_nd_indices = tf.stack([batch_indices, top_k_indices], axis=-1)
      # gather_nd_indices = torch.stack([batch_indices, top_k_indices], dim=-1)
...
def bbox_overlap(boxes, gt_boxes):
    ...
    i_xmin = torch.maximum(bb_x_min, torch.permute(gt_x_min, [0, 2, 1]))
    # i_xmax = tf.minimum(bb_x_max, tf.transpose(gt_x_max, [0, 2, 1]))
    i_xmax = torch.minimum(bb_x_max, torch.permute(gt_x_max, [0, 2, 1]))
    # i_ymin = tf.maximum(bb_y_min, tf.transpose(gt_y_min, [0, 2, 1]))
    i_ymin = torch.maximum(bb_y_min, torch.permute(gt_y_min, [0, 2, 1]))
    # i_ymax = tf.minimum(bb_y_max, tf.transpose(gt_y_max, [0, 2, 1]))
    i_ymax = torch.minimum(bb_y_max, torch.permute(gt_y_max, [0, 2, 1]))
    ...
    u_area = bb_area + torch.permute(gt_area, [0, 2, 1]) - i_area + 1e-8
    ...
    gt_invalid_mask = torch.less(
           torch.max(gt_boxes, -1, keepdim=True), 0.0)
    ...
    padding_mask = torch.logical_or(
          torch.zeros_like(bb_x_min, dtype=torch.bool),
          torch.permute(gt_invalid_mask, [0, 2, 1]))
    # iou = tf.where(padding_mask, -tf.ones_like(iou), iou)
    iou = torch.where(padding_mask, -torch.ones_like(iou), iou)
...
def get_non_empty_box_indices(boxes, filter_size=0):
    ...
    indices = torch.where(torch.logical_and(torch.greater(height, filter_size),
                                      torch.greater(width, filter_size)))
    return indices[:, 0]